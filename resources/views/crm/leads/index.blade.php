            ...
            <div class="mb-6">
                <form method="GET" action="{{ route('leads.index') }}">
                    <div class="flex gap-2">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search leads...') }}" class="flex-1 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        <select name="status" class="rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            ...
                            @foreach(['new','qualified','contacted','proposal_sent','negotiation','won','lost'] as $s)
                        ...
                        @if (request('search') || request('status'))<a href="{{ route('leads.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">{{ __('Clear') }}</a>@endif
                    </div>
                </form>
            </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($leads as $lead)
                                <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('leads.show', $lead) }}'">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $lead->fullName() }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $lead->assignedEmployee?->user?->name ?? '-' }}</td>
                                </tr>
                            @empty
                                <tr><td colspan="5" class="px-6 py-12 text-center"><h3 class="text-sm font-medium text-gray-900">{{ __('No leads found') }}</h3></td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                @if ($leads->hasPages())
                    <div class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                            <p class="text-sm text-gray-700">{{ __('Showing') }} <span class="font-medium">{{ $leads->firstItem() }}</span> {{ __('to') }} <span class="font-medium">{{ $leads->lastItem() }}</span> {{ __('of') }} <span class="font-medium">{{ $leads->total() }}</span> {{ __('results') }}</p>
                            <div>{{ $leads->links() }}</div>
                        </div>
                    </div>
                @endif
